...
const RegisterPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await register(formData.username, formData.email, formData.password, formData.fullName);
        ...
        <div className="h-full flex flex-col md:flex-row gap-8 md:gap-12 rounded-3xl" style={{ height: 'calc(100vh - 96px)' }}>
            ...
            <div className="flex-1 flex items-center justify-center rounded-3xl border border-white/20 md:order-1" style={{ backgroundColor: 'rgb(18, 18, 18)' }}>
                <div className="w-full max-w-md px-6 md:px-12">
                    <h2 className="text-2xl md:text-4xl font-bold text-center text-white mb-6 md:mb-12" style={{ fontFamily: 'Lyon Display, serif' }}>Sign Up</h2>
                <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6" style={{ fontFamily: 'Inter, sans-serif' }}>
                    ...
                    {successMessage && (
                        <div className="p-4 bg-green-500/20 border border-green-500/30 rounded-xl mt-4">
                            <p className="text-green-400 text-center text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>{successMessage}</p>
                        </div>
                    )}
